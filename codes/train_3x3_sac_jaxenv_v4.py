#!/usr/bin/env python3
"""
3x3 SAC V4 — Pure jax.grad over NNX states. No closure retracing.

Key fix vs V3: graphdefs captured as Python globals, NNX states passed as
pure PyTrees to @jax.jit functions. jax.grad computes gradients directly
on the PyTrees.  This eliminates the ~56ms/step retracing bottleneck.

Steady-state: ~27ms per full update step (critic + actor + polyak).
Estimated: ~13 min/seed at 60M steps, 5 seeds ≈ 65 min total.
"""
...
# ---- Pure-JAX SAC update functions ----
# Graphdefs captured ONCE at module level (compile-time constants)
_SAC_ACTOR_GD = None
_SAC_CRITIC_GD = None
_SAC_TCRITIC_GD = None


def _init_graphdefs(actor, critic, tcritic):
    """Capture graphdefs once. Call after model creation."""
    global _SAC_ACTOR_GD, _SAC_CRITIC_GD, _SAC_TCRITIC_GD
    _SAC_ACTOR_GD, _ = nnx.split(actor)
    _SAC_CRITIC_GD, _ = nnx.split(critic)
    _SAC_TCRITIC_GD, _ = nnx.split(tcritic)


@jax.jit
def _critic_loss(critic_st, actor_st, tcritic_st, alpha,
                  obs_b, act_b, rew_b, nobs_b, dones_b, key):
    """Pure JAX: critic_st -> scalar loss."""
    actor = nnx.merge(_SAC_ACTOR_GD, actor_st)
    target = nnx.merge(_SAC_TCRITIC_GD, tcritic_st)
    critic = nnx.merge(_SAC_CRITIC_GD, critic_st)
...
def _actor_loss(actor_st, critic_st, alpha, obs_b, key):
    """Pure JAX: actor_st -> (loss, log_prob_mean)."""
    actor = nnx.merge(_SAC_ACTOR_GD, actor_st)
    critic = nnx.merge(_SAC_CRITIC_GD, critic_st)
...
def train_one_seed(seed: int) -> dict:
    print(f"\n{'='*60}\n# SAC-V4 seed={seed}  N_ENVS={N_ENVS}\n{'='*60}")
    ...
    # Capture graphdefs globally
    _init_graphdefs(actor, critic, tcritic)

    # Extract states as pure PyTrees
    _, actor_st = nnx.split(actor)
    _, critic_st = nnx.split(critic)
    tcritic_st = critic_st  # target starts as copy

    # Optimizers (working on pure PyTrees)
    total_updates = n_iter * N_EPOCHS * max(1, (N_STEPS*N_ENVS)//BATCH_SIZE)
    def _make_opt(lr):
        if LR_DECAY:
            sched = optax.cosine_decay_schedule(lr, total_updates, alpha=LR_END/lr)
    ...
    c_opt = _make_opt(CRITIC_LR); c_opt_st = c_opt.init(critic_st)
    a_opt = _make_opt(ACTOR_LR); a_opt_st = a_opt.init(actor_st)
    ...
    print("  [compiling rollout + updates...]", end="", flush=True)

    # Warmup compile: rollout
    state, obs, rok, _ = rollout_jit(actor, state, obs, rok, N_STEPS)
    jax.block_until_ready(state.gammas)
    # Warmup compile: update functions
    ...
    _ = _critic_loss(critic_st, actor_st, tcritic_st, 0.2, _d_o, _d_a, _d_r, _d_o, _d_d, _d_k)
    _ = _actor_loss(actor_st, critic_st, 0.2, _d_o, _d_k)
    # Also compile gradient functions
    __critic_grad = jax.grad(_critic_loss)
    __actor_grad = jax.grad(lambda *a, **kw: _actor_loss(*a, **kw)[0])
    _ = __critic_grad(critic_st, actor_st, tcritic_st, 0.2, _d_o, _d_a, _d_r, _d_o, _d_d, _d_k)
    _ = __actor_grad(actor_st, critic_st, 0.2, _d_o, _d_k)
    ...
    ep_rets = []
    _critic_grad = jax.grad(_critic_loss)
    _actor_grad = jax.grad(lambda *a, **kw: _actor_loss(*a, **kw)[0])
        ...
        for it in range(n_iter):
            ...
            for _ in range(N_EPOCHS):
                ...
                for _ in range(n_up):
                    ...
                    # Critic
                    cg = _critic_grad(critic_st, actor_st, tcritic_st, alpha,
                                      ob_j, ac_j, rw_j, no_j, dn_j, ck)
                    c_up, c_opt_st = c_opt.update(cg, c_opt_st, params=critic_st)
                    critic_st = optax.apply_updates(critic_st, c_up)
                    cl = _critic_loss(critic_st, actor_st, tcritic_st, alpha,
                                      ob_j, ac_j, rw_j, no_j, dn_j, ck)
                    ...
                    c_losses.append(float(cl)); a_losses.append(float(al))

            # Update NNX modules from states (for next rollout)
            actor = nnx.merge(_SAC_ACTOR_GD, actor_st)
    ...
    # ---- Save ----
    actor_m = nnx.merge(_SAC_ACTOR_GD, actor_st)
    critic_m = nnx.merge(_SAC_CRITIC_GD, critic_st)
    _, actor_state = nnx.split(actor_m)
    _, critic_state = nnx.split(critic_m)
    with open(os.path.join(CKPT_DIR, f"policy_seed{seed}.pkl"), "wb") as f: pickle.dump(actor_state, f)
    with open(os.path.join(CKPT_DIR, f"critic_seed{seed}.pkl"), "wb") as f: pickle.dump(critic_state, f)

    result = dict(seed=seed, total_steps=int(total_steps), ep_returns=ep_rets,
...
if __name__ == "__main__":
    ...
    if len(all_r) > 1:
        with open(os.path.join(CKPT_DIR, "summary.json"), "w") as f:
            json.dump(dict(n_seeds=len(all_r),
                          total_steps=[r["total_steps"] for r in all_r],
