...
CKPT_DIR = os.path.join(_SCRIPT_DIR, "checkpoints_3x3_sac_fair")
os.makedirs(CKPT_DIR, exist_ok=True)

N_SEEDS=5; N_ENVS=4; TOTAL_STEPS=30_000_000; N_STEPS=512; BATCH_SIZE=256
N_EPOCHS=1; REPLAY_SIZE=500_000; ACT_BOUND=10.0; GAMMA=0.99; TAU=0.005
J_HIST=3; ACTOR_LR=3e-4; CRITIC_LR=3e-4; ALPHA_LR=3e-4; MAX_EPISODE_STEPS=200
WIND_MIXTURE=(0.3, 0.3, 0.4)
...
@jax.jit
def _alpha_loss_fn(log_alpha, log_prob_mean, target_entropy):
    """Pre-compiled alpha loss: takes lp and te as explicit args (no closure)."""
    return (-jnp.exp(log_alpha) * (log_prob_mean + target_entropy)).mean()

for seed in range(N_SEEDS):
    print(f'\n# SAC-FAIR seed={seed} N_ENVS={N_ENVS}'); sys.stdout.flush()
    ...
    per_iter=N_STEPS*N_ENVS
    print(f'  obs_dim={od} act_dim={ad} iterations={ni} per_iter={per_iter}'); sys.stdout.flush()
    ...
    print('  [compile rollout...]',end='',flush=True)
    ...
    print('  [compile grads...]',end='',flush=True)
    _do=jnp.zeros((BATCH_SIZE,od)); _da=jnp.zeros((BATCH_SIZE,ad)); _dr=jnp.zeros(BATCH_SIZE); _dd=jnp.zeros(BATCH_SIZE); _dk=jax.random.PRNGKey(0)
    cg=jax.grad(cl); ag=jax.grad(lambda *a,**kw: al(*a,**kw)[0]); alg=jax.grad(_alpha_loss_fn)
    _=cg(cst,ast,tst,0.2,_do,_da,_dr,_do,_dd,_dk); _=ag(ast,cst,0.2,_do,_dk)
    _=alg(la, 0.0, float(te))
    jax.block_until_ready(True); print('done',flush=True)
    ...
    for it in range(ni):
        ...
        for _ in range(N_EPOCHS):
            ...
            for _ in range(nu):
                ...
                # USE PRE-COMPILED alpha grad (no closure!)
                alu,alos=alo.update(alg(la, lp, float(te)), alos, params=la)
    ...
    print(f'# SAC seed={seed} done: {ts/1e6:.1f}M steps  time={r["train_time"]/60:.1f}min  fps={ts/r["train_time"]:.0f}'); sys.stdout.flush()
