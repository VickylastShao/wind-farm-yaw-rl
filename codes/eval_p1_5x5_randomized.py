# -*- coding: utf-8 -*-
"""
Distribution-wise P1 (5x5) evaluation using JAX batched env.

Twin of eval_p0c_randomized.py but for the 5x5 NREL-5MW layout policies
trained via train_5x5_nnx_jaxenv.py (P1 series, OUT_TAG="p1_5x5").

Loads 3 NNX policies and evaluates each on N_CONDITIONS random wind
conditions sampled from the same training distribution
(phi ~ U(173, 353), v ~ U(6, 16)).  Reports the same 4x4 by_dir_v_bin
segmentation as the 3x3 paper headline so results are apples-to-apples.

Outputs:
  latex_draft/figures/p1_5x5_eval_randomized.json
  latex_draft/figures/fig_p1_5x5_eval_randomized.{pdf,jpg}
"""
...
def evaluate_batch(model, positions_jax, phis, vs, baselines, N_turb):
    """Evaluate policy on a batch of (phi, v) conditions via vmap + scan."""
    B = phis.shape[0]
    N_steps = SETTLE_STEPS

    _eval_j_val = int(os.environ.get("J", "1"))
    print(f"  [DEBUG] evaluate_batch J={_eval_j_val}, USE_POSITIONS={os.environ.get('USE_POSITIONS','0')}")
...
def main():
    ...
    print(f"# Distribution-wise P1 5x5 eval (JAX batch)")
    print(f"# layout = {R}x{C} = {N_turb} turbines")
    ...
    summary = dict(
        ...
        out_tag=OUT_TAG,
        # --- reproducibility metadata (peer-review m4) ---
        repro=dict(
            train_script="codes/train_5x5_nnx_jaxenv.py",
            ckpt_template=os.path.join(
                "codes/checkpoints_5x5_nnx_jaxenv",
                f"policy_seed{{seed}}_{OUT_TAG}.pkl"),
            ckpt_files=[f"policy_seed{s}_{OUT_TAG}.pkl"
                        for s in range(N_SEEDS)],
            env_module="codes/windfarm_env_jax.py",
            layout_fn="windfarm_env.create_wind_farm_layout_5x5",
            ...
            j_history=1,
            settle_steps=SETTLE_STEPS,
            yaw_action_range_deg=[-5.0, 5.0],
            yaw_clip_range_deg=[-50.0, 50.0],
        ),
        per_seed=[],
        per_seed_rows=[],
    )

    # Compute obs_dim matching the training configuration.
    ...
    for s in range(N_SEEDS):
        ckpt = os.path.join(CKPT_DIR, f"policy_seed{s}_{OUT_TAG}.pkl")
        model = load_nnx_policy(ckpt, obs_dim, act_dim)
        print(f"\n## seed {s}: ckpt = {os.path.basename(ckpt)}")
    ...
    out_path = os.path.join(FIG_DIR, "p1_5x5_eval_randomized.json")
    ...
    ax = axes[0, 0]
    # Clip the x-axis to the 1st–99th percentile so the dense central mass
    # near 0% (where the +0.23% marginal-mean signal lives) is legible
    # instead of being crushed by ±50% extreme tails. Tail counts are still
    # exposed via the p5/p95 stats_text in the corner (peer-review m2).
    xlo, xhi = np.percentile(cond_mean, [1.0, 99.0])
    pad = 0.06 * (xhi - xlo)
    xlim = (xlo - pad, xhi + pad)
    ax.hist(cond_mean, bins=60, range=xlim,
            color="#E45756", alpha=0.8, edgecolor="black", linewidth=0.3)
    ax.axvline(0, color="black", lw=0.8, ls="--")
    ax.axvline(across["mean_gain_pct"], color="#4C78A8", lw=1.5, ls="-",
               label=f"mean = {across['mean_gain_pct']:+.3f}%")
    ax.set_xlim(xlim)
    ax.set_xlabel("Farm-power gain [%] (1st–99th pctile range)")
    ax.set_ylabel("Count of conditions")
    ax.set_title(f"5×5: Distribution of 3-seed-mean gain\n"
    ...
    ax.set_title("5×5: per-condition gain vs wind direction\n(colored by speed)",
                 fontsize=10)
    ...
    ax.set_title("5×5: gain% vs v, binned by direction-misalignment\n"
    ...
    fig.tight_layout()
    out_fig = os.path.join(FIG_DIR, "fig_p1_5x5_eval_randomized")
